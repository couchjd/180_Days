...
void TriangleShooter::run() {
	...
	while(window.isOpen()) {
		...
		if(clock.getElapsedTime().asSeconds() > 0.05f) {
			...
			if(firing) {
				//newProj = reinterpret_cast<Projectile *>(alloc.Allocate());
				//projectile->setPosition(triangle[1].position, facing(triangle));
				//memcpy(newProj, projectile, sizeof(Projectile));
				Projectile* newProj = new Projectile(triangle[1].position, facing(triangle));
				//std::cout << newProj << std::endl;
